...
<body>
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        $rat=$_POST["rating"];
        $up=pg_query($connection,"select rating from DocumentWriters where WriterID=$id");
        ...
        $rev=$_POST["review"];
        $query="insert into Writer_review(writer_id,rating,review) values($id,$rat,'$rev');";
        $result=pg_query($connection,$query);
    }
    //echo "Connected successfully";
    $query="select * from DocumentWriters where WriterID=$id";
    $result=pg_query($connection,$query);
    //echo "got";
    ...
    $member=$row['writinggenre'];
    $study=$row['qualifications'];
    $address=$row['address'];
    $join=$row['joindate'];
    $rating=$row['rating'];
    $fees=$row['fees'];
    $won=$row['documentswritten'];
    ...
    <div class="main">
        ...
        <div class="right">
            ...
            <div class="heading"> Writing Genre:</div>
            <div class="space">
                <?php echo $member; ?>
            </div> 
    ...
    <div class="main2">
        ...
        <div class="right1">
            <div class="heading">No. of documents Written:</div>
                <div class="space">
                    <?php echo $won; ?>
                </div> 
        </div>
    ...
    <div class="contact">
        ...
        <div class="mid1">
            <div class="heading">Email:</div>
                <div class="space">
                <button><a href="tel:<?php echo $email; ?>"><?php echo $email; ?></a></button>
            </div> 
        </div>
        <div class="right1">
            <div class="heading">Phone Number:</div>
                <div class="space">
                <button><a href="tel:<?php echo $contact; ?>"><?php echo $contact; ?></a></button>
                </div> 
        </div>
    </div><br><br><br>
    <div class="rev">
    ...
    <?php 
    $query = "SELECT review FROM writer_review WHERE writer_id = $id ORDER BY random() LIMIT 2;";
    $result = pg_query($connection, $query);
    ...
    <div style="text-align:center">
            <button type="submit"><?php echo "<a href='writer_review_form.php?id=$id'>Give your review :)</a>";?></button>
    </div><br>
    </form>
    <footer> 
